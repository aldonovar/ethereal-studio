...
const DEFAULT_DIGEST_CHUNK_BYTES = 4 * 1024 * 1024;
...
export class IncrementalSha256 {
  ...
  update(input: Uint8Array): this {
    if (this.finished) throw new Error('La huella SHA-256 ya fue finalizada.');
    if (!(input instanceof Uint8Array)) throw new Error('SHA-256 requiere bytes.');
    this.bytesHashed += input.byteLength;

    let offset = 0;
    while (offset < input.byteLength) {
      const amount = Math.min(SHA256_BLOCK_BYTES - this.blockLength, input.byteLength - offset);
      this.block.set(input.subarray(offset, offset + amount), this.blockLength);
      this.blockLength += amount;
      offset += amount;
...
export async function sha256Blob(
  blob: Blob,
  chunkBytes = DEFAULT_DIGEST_CHUNK_BYTES,
): Promise<string> {
  if (!(blob instanceof Blob) || blob.size <= 0) throw new Error('El archivo está vacío.');
  if (!Number.isSafeInteger(chunkBytes) || chunkBytes <= 0) throw new Error('Tamaño de bloque inválido.');
